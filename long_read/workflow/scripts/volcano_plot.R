...
top_up<-head(RNA[order(RNA$log2FoldChange,decreasing = F),],
             snakemake@params[["number"]])
top_down<-head(RNA[order(RNA$log2FoldChange,decreasing = T),],
               snakemake@params[["number"]])
wgt_top_up<-head(RNA[order(RNA$log2FoldChange*RNA$baseMean,decreasing = F),],
                 snakemake@params[["number"]])
wgt_top_down<-head(RNA[order(RNA$log2FoldChange*RNA$baseMean,decreasing = T),],
                   snakemake@params[["number"]])
